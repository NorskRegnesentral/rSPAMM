...
#' @return results Returns a plot of predicted population size for different population components
#' @keywords population model
#' @export
#' @examples
#' plot.N(res, data)

plot.N <- function(results=res,dat=data,component=c('N0', 'N1'),xLim=NA,yLim=NA)
{
  ...
  if('N1' %in% component) {
    ...
    plot(results$years, results$rep.matrix[results$indN1,1],
         ...
         col=theCols[1], lwd=2, lty=2, xlim=xLim, ylim=yLim)
  ...
  if('N0' %in% component) {
    ...
    if(!'N1' %in% component) {
      ...
      plot(results$years, results$rep.matrix[results$indN0,1],
           type = "l",xlab = "Year",ylab = "Abundance",
           col=theCols[2], lwd=2, lty=2, xlim=xLim, ylim=yLim)
    } else { 
