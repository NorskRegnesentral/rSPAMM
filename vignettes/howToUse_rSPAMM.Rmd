...
To load the data used you run the following command:
```{r eval=FALSE}
...
The optimization routine used is *nlminb*. (MARTIN: bør vi legge inn en mulighet til å endre optimiseringsrutiner). 
...
size is below $N{70}$. Because of this there is no point estimating the $N{70}$ catch level.
...
If the $N{70}$ population size was below the current population size you could re-run the model using the estimated $N{70}$ catch level 
```{r eval=FALSE}
