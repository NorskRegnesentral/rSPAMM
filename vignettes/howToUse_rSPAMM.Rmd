...
plot.N(res,data)
```
![Modelled pup abundance and 1+ abundance.](figures/model_fit.png){ width=98%}
(MARTIN: Kanskje vi skal legge til at vi kan vise Ntot også slik at vi får pups og Ntot? Ser også ut til at N70 og Nlim blir beregnet ut fra N1 og ikke Ntot så dette bør vi kanskje endre på. Bare gi meg et vink så kan jeg ta meg av dette. Skal vi også ha en option der brukeren kan spesifisere farge i tilfelle man noen ganger trenger sort hvitt (eller bare sette en option hvor man velger farge eller gråtone? Mest fleksibelt med å velge farge selv, men man kan gjøre begge deler, altså spesifisere farge selv OG ha en option som "overrider" den hvor man spesifiserer farge eller gråtone). Skal vi legge inn en mulighet for å slå av og på legend - eller modifisere teksten?)
...
If you want to explore the birth ogive data used for various periods you can run the *plot.Pmat()* function:
```{r eval = FALSE}
plot.Pmat(dat = data)
```
...
### The population dynamics model
Martin: kunne lagt modellen her istedet for i begynnelsen. Vet ikke hva som er best.

### Complete code for assessment
Legge til kode som summerer alle stegene for en komplett analyse.